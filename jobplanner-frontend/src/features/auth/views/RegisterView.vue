...
const handleRegister = async () => {
  try {
    ...
    error.value = 'Failed to create account. Email might already be taken.'
  } finally {
...
const handleGoogleLogin = async () => {
  ...
  try {
    ...
    error.value = 'Failed to get Google register URL'
  } finally {
...
const handleMicrosoftLogin = async () => {
  ...
  try {
    ...
    error.value = 'Failed to get Microsoft register URL'
  } finally {
    isConnectingSocial.value = false
  }
}
    ...
    <div class="grid w-full max-w-6xl gap-6 lg:grid-cols-[1.05fr_0.95fr]">
      <Card class="hidden min-h-[640px] overflow-hidden lg:block">
        <CardContent class="flex h-full flex-col justify-between p-10">
          <div>
            ...
            <h1 class="display-title max-w-xl text-6xl font-semibold leading-none">Join the most elegant job hunt.</h1>
            <p class="mt-6 max-w-lg text-base leading-7 text-muted-foreground">
              Create your workspace today and start organizing your career transition with clarity.
            </p>
          </div>
      ...
      <Card class="mx-auto w-full max-w-md">
        <CardHeader class="space-y-3 text-center">
          ...
          <CardTitle class="display-title text-4xl">Get Started</CardTitle>
          <p class="text-sm text-muted-foreground">Create an account to start tracking</p>
        </CardHeader>
        ...
        <CardContent>
          <form class="space-y-6" @submit.prevent="handleRegister">
            ...
            <p class="mt-4 text-center text-sm text-muted-foreground">
              ...
              <router-link to="/auth/login" class="font-medium text-primary hover:underline">Sign in</router-link>
            </p>
          </form>
        </CardContent>
      </Card>
